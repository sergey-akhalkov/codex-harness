...
## Предлагаемая последовательность дальнейших исследований

1. Оценить необходимость других платформ после первой реализации для Windows.
2. Использовать перенесённое глобальное ядро и конкретизировать критерии оценки качества/скорости для следующих возможностей.
3. Проверить загрузку одного skill и одной полезной MCP-интеграции в отдельном проекте.
4. Сравнить baseline и усиленный вариант на реальных задачах.
5. По результатам переносить агентов, hooks, память и другие возможности.

Это предложение по направлению работы. Оно не создаёт OpenSpec change, не фиксирует обязательный roadmap и не разрешает установку компонентов.
