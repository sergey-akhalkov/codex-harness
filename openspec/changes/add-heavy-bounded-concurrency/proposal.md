...
- Keep one aggregate memory envelope per account: all admitted trees are members of one account aggregate Job with the account memory limit, while each tree keeps its own containment Job with per-tree memory, deadline, cancellation and cleanup.
...
- `crates/harness-core/src/resource_admission.rs`: account admission grows from one exclusive lease to a bounded slot set with queue diagnostics.
- `crates/harness-core/src/heavy_command.rs` and `crates/harness-core/src/process.rs`: account aggregate Job creation/membership and snapshot reporting while preserving the documented admission/lock order.
