...
The concurrent `add-observed-executor-tui` change owns native presentation and the full base `Observable executor lifecycle and bounded result` requirement. This change adds reply holds and a uniquely named waiting-observation requirement instead of replacing that block. The concurrent `close-failed-executor-tab` change owns closing a finished tab and preserving the real outcome separately from the terminal host exit. Keep the selected native frontend attached while a question is pending, close it only at an actual run end, and do not reintroduce a custom renderer or change terminal exit policy here. The [simplification design](../2026-09-26-simplify-native-harness/design.md) owns cross-cutting consolidation and instruction reduction; consume its native owner rather than introducing another controller. These distinct delta operations must preserve each other's scenarios on archive.
...
Installed route check for task 1.1, `codex-cli 0.156.1`: the owned contract test addressed the exact app-server thread through that server's endpoint and the canned Responses fixture. Delivery required the marker in the owning thread's items and in a later provider request for that same thread. A native item event carried the same marker. Queue exit 0, the text `Queued message`, and an RPC without `error` were recorded and were not treated as delivery. Help text was not used as evidence.

On 0.156.1, `codex queue --thread <exact id> --remote <owning endpoint>` accepts during active generation and while a tool is blocked, then the marker reaches a later turn of the same thread. That is not timely. `turn/steer` with the active turn id puts the marker in that turn's next provider request before the turn completes. The blocked-tool case holds the canned fixture's tool until its observation release file is written; the fixture does not emit `executor watch`, and this check does not spawn an executor. The turn stayed active through injection. On an idle loaded thread, both queue and `turn/start` reach that thread and its next provider request.
...
Task 1.2 exercised that discovery through the executor shell path on 2026-09-25, without a messaging service, a second copy of a live thread, or window layout. No launcher change is required for attribution, and none is made here.

A lead's own tool shell carries that lead's `CODEX_THREAD_ID` and `CODEX_SESSION_ID`. Two leads are distinct even when they share a terminal: the thread id is the identity, not the checkout, window, or label. Dispatch then starts the executor host through `apply_host_env`, which clears those two variables before the host process exists. The observed host and its app-server parent therefore have neither variable. The executor's later tool shell receives a new thread id from its own app-server. That id is the executor session, not the originating lead, and must not be used as the return address. After a terminal-tab dispatch the host's parent is the terminal, not the lead, so walking parents from the executor shell cannot choose between two leads in one window.

The dispatch receipt records the executor profile, slot, owner, source checkout, and the executor's own session and control endpoint. It does not record the originating lead or a lead endpoint. A later caller is attributed by capturing the dispatching shell's thread id before that clear, storing it on the run, and checking the caller's membership in that run's process tree. Capture belongs to the spawn relationship in task 2.1. The shell path itself does not discover it.
...
- [Launcher integration changes normal use] -> Task 1.2 exercised the ordinary installed lead path and explicit profiles. They expose no listen endpoint, and task control stays off because it lays out windows and bypasses profiles. Explicit remote attachment already preserves caller arguments. Do not add a test-only lead launcher.
