...
- Spawn every executor as a native `codex --profile <id>` session in its own visible terminal window and a Codex-managed Git worktree before its first model request. Allocation follows installed CLI managed worktrees (`--worktree`, experimental `worktrees` feature). The controller records the native mapping, including owner-thread identity from CLI 0.155.0, and retires the checkout after merge or explicit discard. CLI allocations are still not auto-cleaned; 0.155.0 confirmed deletion applies only to a clean managed worktree of the current repository that is not the current checkout and has no local, untracked or ignored changes. When that native deletion does not apply, preserve the checkout and report the limit instead of force-removing it or treating agents-overview hide/archive/task deletion as retirement. Ordinary Git worktrees from `isolated-worktree-workflow` are not a substitute. The verified `--remote` control view does not pass `--worktree` (native rejects that combination) and attaches only after the session cwd is the managed checkout. Executors never write to the shared checkout. Do not invent a second harness worktree tree.
