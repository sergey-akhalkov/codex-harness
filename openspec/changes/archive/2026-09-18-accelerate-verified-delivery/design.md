...
- Establish six to eight reusable outcome cases and honest local evidence, including failures and coordination costs; keep quantitative benefit unproven for this change.
...
- Requiring a two-consumer quantitative speed comparison, or claiming Astra/Grok acceleration, to close this change.
...
The eight cases below record the original experiment. On 2026-09-08 the user withdrew ongoing support for the original source-kit consumer and further spending on it; case 1 and its completed runs are historical, with no further run of the original source-kit consumer required. This overrides the original consumer selection and repetition plan below. On 2026-09-18 the user decided that the two-consumer quantitative benchmark is not required for this change. The historical 15-second / 15% threshold is not lowered retroactively; benefit remains unproven and is not an implementation-acceptance requirement. Further model-backed two-consumer runs are out of scope here.
...
The accepted capability selection also keeps Fast off, native memories off, CBM automatic index/watch off, and OpenAI assignments Astra-only. One native baseline/candidate pair later ran for each remaining local controlled case (freshness, entrypoint, process, missing, negative) on that hooks-off Astra/xhigh selection, with independent oracles and private evidence. Historical source-kit, user-gated second-consumer and withdrawn-wrapper cases stayed excluded. Benefit remains unproven. The 2026-09-18 decision does not authorize further runs of the original source-kit consumer and does not authorize further model-backed two-consumer spending under this change.

Two real consumers outside this checkout were named locally for qualitative consumption. Freeze them only through explicit `--primary` and `--secondary` paths into `tools/outcome_cases.py --prepare`. The tracked runner stores generic `primary`/`secondary` snapshot keys and must not discover neighboring checkouts by sibling directory name or write consumer identities into Git. Local controlled cases remain the five pairs plus documented native commands already exercised; the original source-kit consumer and its withdrawn wrapper stay excluded. Live checkouts stay untouched. If a later change reopens two-consumer measurement, pin hooks-off Astra/xhigh in both arms, keep documented commands docs-only until execution, and do not lower the historical 15-second / 15% verified-time threshold or allow correctness loss after seeing results. This change does not run that comparison. Detailed JSON stays private; a synthetic fixture cannot replace a real consumer.
...
- **Dependency timing:** skills can progress independently. Historical diagnostic-hook repair remains archived evidence and is not a restoration prerequisite. Task 5.3 completed the remaining local controlled pairs. Tasks 5.4 and 6.4 close on the 2026-09-18 scope decision rather than a two-consumer proof. Ordinary hooks stay off.
...
5. Activate the accepted source-linked skills globally and verify fresh outside-project sessions. If activation fails, use the existing owned-link rollback. After the 2026-09-18 scope decision, close implementation with benefit unproven; do not keep 5.4/6.4 open waiting for a two-consumer speed proof.

Planning created no global registrations, runtime changes or model-backed evaluation runs. After the 2026-09-18 decision, 5.4 records that scope and 6.4 validates; they do not wait for a two-consumer quantitative proof.
