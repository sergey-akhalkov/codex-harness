...
The accepted capability selection also keeps Fast off, native memories off, CBM automatic index/watch off, and OpenAI assignments Astra-only. One native baseline/candidate pair later ran for each remaining local controlled case (freshness, entrypoint, process, missing, negative) on that hooks-off Astra/xhigh selection, with independent oracles and private evidence. Historical source-kit, user-gated second-consumer and withdrawn-wrapper cases stayed excluded. Benefit remains unproven. The remaining quantitative decision is whether to retain a two-consumer benefit benchmark after support for the original source-kit consumer was withdrawn; that decision is pending, does not authorize further runs of the original source-kit consumer, and does not authorize further spending on a second real consumer until one is chosen.

Two real consumers outside this checkout were named locally. Freeze them only through explicit `--primary` and `--secondary` paths into `tools/outcome_cases.py --prepare`. The tracked runner stores generic `primary`/`secondary` snapshot keys and must not discover neighboring checkouts by sibling directory name or write consumer identities into Git. Benefit cases remain the five local controlled pairs plus one documented native command from each named consumer; the original source-kit consumer and its withdrawn wrapper stay excluded. Live checkouts stay untouched. Pin hooks-off Astra/xhigh in both arms, keep documented commands docs-only until execution, and repeat each benefit case at least twice per arm with alternating order. Do not lower the predeclared 15-second / 15% verified-time threshold or allow correctness loss after seeing results. Detailed JSON stays private; a synthetic fixture cannot replace either consumer.
