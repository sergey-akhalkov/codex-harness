...
The accepted capability selection also keeps Fast off, native memories off, CBM automatic index/watch off, and OpenAI assignments Astra-only. Withdrawn support for the original source-kit consumer SHALL NOT be resumed. Incomplete quantitative benefit tasks remain open until the pending two-consumer decision is made.
