...
- Add instruction-refresh succession: a session whose instructions or skills changed spawns a successor through a deterministic `codex resume` profile session at a safe boundary after in-flight tool effects, hands over durable context, then stops its own CLI process; resumed sessions must be verified to reload current instructions and skills.
...
- Gate adopted improvements on matched comparisons of quality and speed per token burn, including feedback-triage, coordination and rework cost.
...
continues to own the mechanics of evolving skills and instructions; this
change owns the intake queue, promotion and succession that consume their
updates. Private consuming-project data stays outside kit sources.
