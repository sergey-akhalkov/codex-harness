...
Confirmed user decisions: the language policy applies only to development of `codex-harness`; third-party tools may retain their languages/runtimes; the target for this migration is Rust for all maintained first-party executable code. Earlier discussion of retaining PowerShell is not an exception to that target. The user values execution speed and the full cost/time of achieving a verified result. This change records those decisions; implementation will update the local owning decision record and repository guidance, without publishing a language mandate to other projects.
