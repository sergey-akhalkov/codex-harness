...
- Adopt the CodeGraph replacement owned by [replace-cbm-with-codegraph](../archive/2026-09-11-replace-cbm-with-codegraph/proposal.md), including automatic refresh in all concurrently open CLI projects and shared resources across clients of one project. That change owns the first-party Rust graph adapter and its provider acceptance; this migration integrates the same implementation and evidence. Do not continue a separate CBM port, optimize its indexer or rewrite either upstream tool. Existing CBM paths are transitional baseline/rollback material until verified retirement.
