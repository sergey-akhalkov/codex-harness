...
- [ ] 1.1 Reproduce and pin the defect: retain the observed startup failures
...
- [ ] 1.2 Share a fixed anchor read bound in `broker_state` and use it for the
       Serena broker, CodeGraph account and read-only CodeGraph location
       lookups.
- [ ] 1.3 Make generation selection bounded: drop vanished locations, prune
...
- [ ] 2.1 Add unignored regression tests: a grown CodeGraph account record and
...
- [ ] 2.2 Add unignored chooser tests: dead-root reuse without preparing a new
       root, live-generation preservation, bounded retained records across
       successive sources.
- [ ] 2.3 Extend adopted-package MCP acceptance with a pre-grown anchor: the
...
- [ ] 3.2 Update the owning documentation for the bounded generation-record
