...
def resolve_launch(server: str, inventory: Inventory, catalogue: Catalogue) -> tuple[list[str], dict[str, str]]:
    if server in ('harness-lsp', 'codebase-memory', 'graphify'):
        raise RuntimeError(f'{server} is retired; managed MCP servers are serena, codegraph and nuphus')
    ...
    if server in ('serena', 'nuphus') and not Path(command[2]).is_file():
...
def main() -> None:
    ...
    _ = parser.add_argument('server', choices=['serena', 'nuphus'])
