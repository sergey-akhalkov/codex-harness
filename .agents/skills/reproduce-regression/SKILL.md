...
For process fixtures, read [process ownership and optional helper](references/process-fixtures.md). Use native `harness-observe.exe` for bounded execution, concurrent stream capture, observable readiness and owned cleanup. When it is unavailable, follow the documented fixture contract with the consuming project's own tooling; do not add a framework merely to reproduce a defect.
