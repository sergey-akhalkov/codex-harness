...
For process fixtures, read [process ownership and optional helper](references/process-fixtures.md). Prefer native `harness-observe.exe` for bounded execution, concurrent stream capture, observable readiness and owned cleanup. Linked Python/PowerShell helpers remain transitional when the native helper is unavailable; do not add a framework merely to reproduce a defect.
