...
- A bounded automatic update can satisfy freshness. Do not trigger a full index or manual sync before every query. During debounce or a failed update use a current scoped read; make one deliberate sync only when needed. CBM's current manual-refresh policy remains separate until replacement.
