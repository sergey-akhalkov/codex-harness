...
Give an isolated worker its actual root, base, transferred inputs, file ownership, required skills, invariants and acceptance commands. Before the chosen MCP operation, verify its active checkout: activate the intended root in Serena or verify the selected graph provider's root, freshness and coverage. Do not initialize every provider as a checklist. Refresh unknown or changed graph evidence, or reuse a verified completed automatic update where supported. If coverage or selection cannot be established, state the limitation and use fresh scoped source; a parent graph is not evidence about the worktree. Apply the task-based retrieval budgets to child results as well.
