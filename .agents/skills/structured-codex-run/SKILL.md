...
For the bundled contract, use [inspection.schema.json](assets/inspection.schema.json) and native `harness-inspect.exe`; read [the invocation contract](references/invocation.md) for flags, schema, limits and evidence files. Prefer the verified native helper. The Python [helper](scripts/run.py) remains transitional until first-party scripts are retired. Invoke a model only within the user's authorized route and conversation-visibility policy.
