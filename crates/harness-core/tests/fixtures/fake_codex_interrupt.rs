//! Owned Codex double that hangs on --version after journal publication.
//! Feature commands stay model-free and return immediately.
use std::{
    env, fs,
    io::{self, Write},
    path::PathBuf,
...
fn hang_version() {
    if let Some(root) = env::var_os("HARNESS_CORE_INTERRUPT_ROOT") {
        let marker = PathBuf::from(root).join("paused-version");
        fs::write(&marker, b"version").unwrap_or_else(|_| fail(1));
