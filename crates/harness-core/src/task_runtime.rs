...
/// Membership of one long-lived session owner in the account-wide CPU
/// allowance.
///
/// The allowance is scoped to the Windows account, so sessions started from
/// different checkouts, Codex homes, projects and terminal tabs join the same
/// object; the owner keeps this handle for its whole process lifetime. A Job
/// object outlives its handles while members remain, so closing one owner's
/// handle neither lifts a peer's ceiling nor kills a peer's work.
#[derive(Debug)]
...
impl SessionCpuAllowance {
    ...
    /// own allowance, the persistent default is not disabled, and no payload
    /// starts twice.
    ///
    ...
    fn join_at(scope: &str, percent: f64) -> Self {
        ...
        // assigned to the object it already belongs to.
...
/// Admit one payload to the account allowance (outer) and to its owner's
/// lifecycle Job (inner) before it executes. The kernel assigns the complete
/// ordered list at creation and the process owner verifies it before returning,
/// so a successful call is verified membership rather than an assumption, and
/// the lifecycle Job keeps its exclusive cleanup authority.
///
/// Availability is preserved on an admission failure: the requested payload
/// still starts once with its ordinary lifecycle, and the report says that any
/// ceiling it has is unverified instead of claiming the shared one.
pub fn spawn_admitted(
    ...
    let Some(budget) = budget else {
        ...
        // account allowance; the payload keeps its ordinary lifecycle and is
        // never claimed as capped.
...
fn member_of_allowance(name: &str) -> io::Result<bool> {
    ...
    current_in_job(handle)
}

/// Assign this process to the named CPU-only allowance when the kernel allows
/// it, then confirm the membership. Assignment needs only the documented assign
/// right and grants no termination or configuration authority over the object
/// or over peers; a refusal is reported, never retried.
...
fn current_in_job(job: std::os::windows::io::OwnedHandle) -> io::Result<bool> {
    use std::os::windows::io::AsRawHandle;
    use windows_sys::Win32::System::JobObjects::IsProcessInJob;
    use windows_sys::Win32::System::Threading::GetCurrentProcess;
    let mut member = 0;
    if unsafe { IsProcessInJob(GetCurrentProcess(), job.as_raw_handle().cast(), &mut member) } == 0
    {
        return Err(io::Error::last_os_error());
    }
    Ok(member != 0)
...
pub fn run(command: &Command, manager: &Path, home: &Path) -> io::Result<Option<i32>> {
    ...
    // This session owner is started by a terminal tab or by a dispatch route
    // outside the account allowance, so it joins the shared account CPU
    // allowance before the service, the native server or any visible
    // conversation exists. The handle is retained for the whole session and
    // every descendant inherits the ceiling, while the terminal tab that
    // started this owner stays outside it.
    let _allowance = SessionCpuAllowance::join("this task-control session owner");
