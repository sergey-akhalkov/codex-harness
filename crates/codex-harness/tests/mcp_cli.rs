...
fn recorded_mcp_runtime_rejects_stale_source_while_management_remains_available() {
    ...
    for directory in [source.join("crates/one/src"), source.join("tools/rtk-adapter/src"),
        source.join(INSPECTION_SCHEMA).parent().unwrap().into(), build.clone()] {
        fs::create_dir_all(directory).unwrap();
    }
    for file in ["Cargo.toml", "Cargo.lock", "crates/one/src/lib.rs", INSPECTION_SCHEMA] {
    ...
    for name in BINARIES {
        ...
        if *name != "codex-harness.exe" { fs::write(&path, name).unwrap(); }
        binaries.insert((*name).to_owned(), build_identity::hash_file(&path).unwrap());
    }
    // Fabricated owned identity metadata tests the gate; this is not release-build acceptance.
    let record = BuildRecord { schema: SCHEMA, source_root: source.clone(),
        source: build_identity::source_identity(&source).unwrap(), rustc: "fixture".into(), cargo: "fixture".into(),
        target: "x86_64-pc-windows-msvc".into(), profile: "release".into(), binaries };
    fs::write(build.join("build.json"), serde_json::to_vec(&record).unwrap()).unwrap();
    let invoke = || Command::new(&manager).current_dir(root.path())
        .args(["mcp", "codebase-memory", "--help"]).output().unwrap();
    let healthy = invoke();
    ...
    let stale = invoke();
    assert_eq!(stale.status.code(), Some(2), "source-stale MCP runtime was admitted");
    assert!(stale.stdout.is_empty());
    assert!(String::from_utf8_lossy(&stale.stderr).contains("ordinary runtime is disabled"));
    let check = Command::new(&manager).current_dir(root.path()).args(["check", "--build"])
