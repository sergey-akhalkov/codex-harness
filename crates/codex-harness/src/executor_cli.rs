...
const USAGE: &str = concat!(
    ...
    "  Content: pipe standard input first (or --text - explicitly); short --text and UTF-8 --file remain fallbacks. Long content spills automatically to a harness file whose pointer and receipt report size and path (delivery=spill, payloadPath, payloadBytes). Address fields are optional: harness records resolve the sole live run; ambiguity asks for --slot, and explicit values win and verify unchanged.\n",
    ...
    "Exit codes: 0 stopped, already-stopped or already-completed, 1 error or refusal with nothing terminated, 2 partial stop; invalid options and an address that names another owner or session are refused with the kit's error exit before anything is acted on. --timeout bounds the whole stop path (default 30 seconds). `executor message` delivers one literal correction into the addressed run's own live conversation under the content and address rules above, then verifies the recorded binding, exact session, lease, host and conversation before acting. `--reply-to MESSAGE_ID` is the lead's reply to one request from its own executor and takes only that id plus content. The id resolves from the lifecycle record written by `lead message`, then this same steer/start owner delivers the unchanged payload to that exact executor run. Another lead, an unknown or retired id, a reused slot or generation, and a contradictory explicit address are refused before anything is sent. ",
...
fn drive_control(
    ...
    loop {
        ...
        if let Some(frontend) = frontend
            ...
            && frontend_view_lost(frontend.is_running()?)
        {
            // A terminal event can arrive in the same moment as the exit.
            // Drain it once before deciding. Process exit is not a result.
            if !exit_drained {
                exit_drained = true;
                continue;
            }
            if let Some(outcome) = retained_native_outcome(conversation, receipt) {
                return Ok((outcome, streamed_message));
            }
...
/// Process exit of the only attached frontend is view loss. Focus, tab
/// selection and tiling are not: an unfocused or unselected tab remains the
/// surface.
fn frontend_view_lost(process_running: bool) -> bool {
    !process_running
}

/// The accepted turn's own recorded status, when that status is already
/// terminal. A missing read, another turn, or a non-terminal status is not a
/// result, and frontend exit never manufactures one. An unresolved reply hold
/// keeps a completed turn from settling as success.
fn retained_native_outcome(conversation: &mut Conversation, receipt: &Path) -> Option<Lifecycle> {
    let turn_id = conversation.turn()?.turn_id.clone();
    let thread = conversation.thread_state().ok()?;
    let status = thread
        .get("turns")
        .and_then(serde_json::Value::as_array)
        .and_then(|turns| {
    ...
    match status {
        "completed" if completed_may_settle(receipt) => Some(Lifecycle::Completed),
        "failed" => Some(Lifecycle::Failed),
        "interrupted" => Some(Lifecycle::Interrupted),
        _ => None,
    }
}

/// A receipt that cannot be read does not prove a reply hold is gone. Leaving
/// the completed turn unsettled is safer than reporting success.
fn completed_may_settle(receipt: &Path) -> bool {
    ...
    !observation::unresolved_reply_hold(&value)
}

/// The only frontend exited during active work. Interrupt the accepted turn,
/// contain the owned backend, and record interruption unless a terminal result
/// is already retained. No further assignment is submitted.
fn contain_frontend_loss(
    frontend: &mut Option<OwnedFrontend>,
    conversation: &mut Conversation,
    receipt: &Path,
    tracker: &mut RunTracker,
    ...
    let cause = format!(
        "the owned native frontend exited while the assignment was active; further model dispatch was suspended and the owned backend was contained (interrupt requested: {interrupted}). Frontend exit is not success. Remedy: executor resume reopens this session with a frontend; {cleanup}"
    );
    let retained = observation::commit_frontend_loss(receipt, tracker, cause.clone())?;
    if retained {
        let exit = tracker.observation.exit_code.unwrap_or(0);
        note_log(
            &plan.paths.log,
            &format!(
                "frontend exit kept the retained result state={} exit={exit}",
                tracker.observation.state
            ),
        );
        return Ok(exit);
    ...
    let mut stdout = io::stdout();
    let _ = writeln!(stdout, "result: interrupted: {cause}");
    ...
    let _ = writeln!(
        stdout,
        "remedy: executor resume reopens this session with a frontend; the slot keeps its binding and partial work, and no fallback run was started"
    );
    let _ = stdout.flush();
    note_log(&plan.paths.log, &cause);
...
mod tests {
    ...
    fn free_text_assignment_renders_the_executor_assignment_guidance() {
        ...
        assert!(
            text.contains("by piping codex-harness lead message"),
            "{text}"
        );
        assert!(
    ...
    fn unfocused_or_unselected_tab_is_not_view_loss() {
        assert!(
            ...
            "a live frontend stays a valid surface when it is unfocused or unselected"
        );
        assert!(frontend_view_lost(false));
    }
    ...
    /// The installed help is the reference for the exchange: the one command an
    /// executor needs, the lead's reply form, run-supplied addressing and the
    /// action-required watch result, so the help and the dispatched guidance
    /// teach the same rule instead of two workflows.
    #[test]
    fn usage_names_the_lead_exchange_and_the_action_required_watch_result() {
        ...
        for taught in [
            "pipe standard input first",
            "--text - explicitly",
            "Long content spills automatically",
            "delivery=spill, payloadPath, payloadBytes",
            "Address fields are optional",
            "ambiguity asks for --slot",
        ] {
        ...
        assert!(
            USAGE.contains("Exit 3 is action required"),
            "the usage must explain the action-required watch result"
        );
        assert!(
            USAGE.contains("Routine progress stays on the beads board"),
            "the usage must keep durable progress on the board"
        );
    }
