    ...
    try:
        ...
        if args.scenario in ("levels", "all"):
            prompt = ("Bounded native level configuration acceptance, not routine production escalation. "
                ...
                "three ordinary agent_type default agents with model gpt-6-astra, fork_context=false, "
                "and reasoning_effort high, xhigh, max respectively. Ask high to compute 17*19 "
                "and return BACKUP_OK:323; ask xhigh whether replacing x with x+1 commutes with "
                "doubling x, return SENIOR_OK plus a counterexample; ask max for a minimal interleaving explaining why "
