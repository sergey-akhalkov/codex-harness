...
foreach ($scenario in @('valid','runtime-expanded-models','api-keys','plaintext-zai-key','role-token','implicit-search','implicit-vision','wrong-middle','recursive-middle','unverified-grok-wire','no-empty-recovery','no-terminal-recovery','picker-allowlist')) {
    ...
    if ($scenario -eq 'runtime-expanded-models') {
        # OpenCodex reconciles OAuth provider presets and persists discovery
        # state into the linked config at every proxy start; source validation
        # must accept that runtime-maintained shape while selectedModels stays
        # the ordinary-picker gate.
        $candidate = $cleanConfig | ConvertFrom-Json -AsHashtable
        $candidate.providers.xai.models = @('grok-4.6','grok-4.5','grok-4.3')
        $candidate.modelDiscovery = @{ knownModels = @{ xai = @{ ids = @('grok-4.6','grok-4.5','grok-4.3'); removed = @(); updatedAt = '2026-09-11T17:43:08.330Z' } } }
        [IO.File]::WriteAllText($configPath, ($candidate | ConvertTo-Json -Depth 30))
    }
    if ($scenario -eq 'plaintext-zai-key') {
        $config = $cleanConfig | ConvertFrom-Json -AsHashtable
        $config.providers.zai.apiKey = 'synthetic-secret-must-not-print'
        [IO.File]::WriteAllText($configPath, ($config | ConvertTo-Json -Depth 30))
    }
    ...
    if ($scenario -eq 'picker-allowlist') {
        $candidate = $cleanConfig | ConvertFrom-Json -AsHashtable
        $candidate.fastRows = $true
        $candidate.providers.xai.selectedModels = @('grok-4.6','grok-4.5')
        [IO.File]::WriteAllText($configPath, ($candidate | ConvertTo-Json -Depth 30))
    }
    ...
    $expected = if ($scenario -in @('valid','runtime-expanded-models')) { 0 } else { 1 }
